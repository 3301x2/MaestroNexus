#!/bin/bash
# MaestroNexus beforeShellExecution hook for Cursor
# Receives JSON on stdin with { command, cwd, timeout }
# Returns JSON on stdout with { permission, agent_message }
#
# Extracts search pattern from grep/rg commands, runs maestronexus augment,
...
# Run maestronexus augment
RESULT=$(npx -y maestronexus augment "$PATTERN" 2>/dev/null)
