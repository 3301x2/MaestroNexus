/**
 * Stateless HTTP client for the local MaestroNexus backend server.
...
const fetchWithTimeout = async (
  ...
  try {
    ...
    if (error instanceof TypeError) {
      throw new Error(`Network error reaching MaestroNexus backend at ${backendUrl}: ${error.message}`);
    }
    throw error;
  } finally {
    clearTimeout(timer);
