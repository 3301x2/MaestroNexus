#!/usr/bin/env node
/**
 * MaestroNexus Claude Code Plugin Hook
 *
 * PreToolUse handler — intercepts Grep/Glob/Bash searches
 * and augments with graph context from the MaestroNexus index.
 *
...
/**
 * Check if a directory (or ancestor) has a .maestronexus index.
 */
function findMaestroNexusIndex(startDir) {
  let dir = startDir || process.cwd();
  for (let i = 0; i < 5; i++) {
    if (fs.existsSync(path.join(dir, '.maestronexus'))) {
      return true;
    }
...
function main() {
  try {
    ...
    if (!findMaestroNexusIndex(cwd)) return;
    ...
    // Try direct maestronexus binary first (faster if globally installed)
    try {
    ...
    if (!result || !result.trim()) {
      try {
        const child = spawnSync(
          'npx',
          ['-y', 'maestronexus', 'augment', pattern],
