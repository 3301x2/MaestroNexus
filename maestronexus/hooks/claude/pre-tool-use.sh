#!/bin/bash
# MaestroNexus PreToolUse hook for Claude Code
...
# Check if we're in a MaestroNexus-indexed repo
dir="${CWD:-$PWD}"
found=false
...
# Run maestronexus augment — must be fast (<500ms target)
RESULT=$(cd "$CWD" && npx -y maestronexus augment "$PATTERN" 2>/dev/null)
