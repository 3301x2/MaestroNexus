 ...
 * All resources use repo-scoped URIs: maestronexus://repo/{name}/context
 */
...
export function getResourceDefinitions(): ResourceDefinition[] {
  return [
    ...
    {
      uri: 'maestronexus://setup',
      name: 'MaestroNexus Setup Content',
...
export function getResourceTemplates(): ResourceTemplate[] {
  return [
    {
      uriTemplate: 'maestronexus://repo/{name}/context',
      name: 'Repo Overview',
      description: 'Codebase stats, staleness check, and available tools',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'maestronexus://repo/{name}/clusters',
      name: 'Repo Modules',
      description: 'All functional areas (Leiden clusters)',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'maestronexus://repo/{name}/processes',
      name: 'Repo Processes',
      description: 'All execution flows',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'maestronexus://repo/{name}/schema',
      name: 'Graph Schema',
      description: 'Node/edge schema for Cypher queries',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'maestronexus://repo/{name}/cluster/{clusterName}',
      name: 'Module Detail',
      description: 'Deep dive into a specific functional area',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'maestronexus://repo/{name}/process/{processName}',
      name: 'Process Trace',
...
function parseUri(uri: string): { repoName?: string; resourceType: string; param?: string } {
  if (uri === 'maestronexus://repos') return { resourceType: 'repos' };
  if (uri === 'maestronexus://setup') return { resourceType: 'setup' };

  // Repo-scoped: maestronexus://repo/{name}/context
  const repoMatch = uri.match(/^maestronexus:\/\/repo\/([^/]+)\/(.+)$/);
  if (repoMatch) {
...
async function getReposResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return 'repos: []\n# No repositories indexed. Run: maestronexus analyze';
  }
  ...
  if (repos.length > 1) {
    ...
    lines.push(`# maestronexus_search({query: "auth", repo: "${repos[0].name}"})`);
  }
...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  if (!context) {
    return 'error: No codebase loaded. Run: maestronexus analyze';
  }
  ...
  lines.push('');
  lines.push('re_index: Run `npx maestronexus analyze` in terminal if data is stale');
  lines.push('');
  lines.push('resources_available:');
  lines.push('  - maestronexus://repos: All indexed repositories');
  lines.push(`  - maestronexus://repo/${context.projectName}/clusters: All functional areas`);
  lines.push(`  - maestronexus://repo/${context.projectName}/processes: All execution flows`);
  lines.push(`  - maestronexus://repo/${context.projectName}/cluster/{name}: Module details`);
  lines.push(`  - maestronexus://repo/${context.projectName}/process/{name}: Process trace`);
...
async function getClustersResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.clusters || result.clusters.length === 0) {
      return 'modules: []\n# No functional areas detected. Run: maestronexus analyze';
    }
    ...
    if (result.clusters.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.clusters.length} modules. Use maestronexus_query for deeper search.`);
    }
...
async function getProcessesResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.processes || result.processes.length === 0) {
      return 'processes: []\n# No processes detected. Run: maestronexus analyze';
    }
    ...
    if (result.processes.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.processes.length} processes. Use maestronexus_query for deeper search.`);
    }
...
function getSchemaResource(): string {
  return `# MaestroNexus Graph Schema
 ...
 * Useful for `maestronexus setup` onboarding or dynamic content injection.
 */
async function getSetupResource(backend: LocalBackend): Promise<string> {
  const repos = await backend.listRepos();

  if (repos.length === 0) {
    return '# MaestroNexus\n\nNo repositories indexed. Run: `npx maestronexus analyze` in a repository.';
  }
  ...
  for (const repo of repos) {
    ...
    const lines = [
      `# MaestroNexus MCP — ${repo.name}`,
      '',
      `This project is indexed by MaestroNexus as **${repo.name}** (${stats.nodes || 0} symbols, ${stats.edges || 0} relationships, ${stats.processes || 0} execution flows).`,
      '',
      '## Tools',
      '',
      ...
      '## Resources',
      '',
      `- \`maestronexus://repo/${repo.name}/context\` — Stats, staleness check`,
      `- \`maestronexus://repo/${repo.name}/clusters\` — All functional areas`,
      `- \`maestronexus://repo/${repo.name}/processes\` — All execution flows`,
      `- \`maestronexus://repo/${repo.name}/schema\` — Graph schema for Cypher`,
    ];
