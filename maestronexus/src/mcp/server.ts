...
import { MAESTRONEXUS_TOOLS } from './tools.js';
...
function getNextStepHint(toolName: string, args: Record<string, any> | undefined): string {
  ...
  switch (toolName) {
    case 'list_repos':
      return `\n\n---\n**Next:** READ maestronexus://repo/{name}/context for any repo above to get its overview and check staleness.`;
    ...
    case 'context':
      return `\n\n---\n**Next:** If planning changes, use impact({target: "${args?.name || '<name>'}", direction: "upstream"${repoParam}}) to check blast radius. To see execution flows, READ maestronexus://repo/${repoPath}/processes.`;

    case 'impact':
      return `\n\n---\n**Next:** Review d=1 items first (WILL BREAK). To check affected execution flows, READ maestronexus://repo/${repoPath}/processes.`;

    case 'detect_changes':
      return `\n\n---\n**Next:** Review affected processes. Use context() on high-risk changed symbols. READ maestronexus://repo/${repoPath}/process/{name} for full execution traces.`;

    case 'rename':
      return `\n\n---\n**Next:** Run detect_changes(${repoParam ? `{repo: "${repo}"}` : ''}) to verify no unexpected side effects from the rename.`;

    case 'cypher':
      return `\n\n---\n**Next:** To explore a result symbol, use context({name: "<name>"${repoParam}}). For schema reference, READ maestronexus://repo/${repoPath}/schema.`;
    ...
    case 'overview':
      return `\n\n---\n**Next:** To drill into an area, READ maestronexus://repo/${repoPath}/cluster/{name}. To see execution flows, READ maestronexus://repo/${repoPath}/processes.`;
...
export async function startMCPServer(backend: LocalBackend): Promise<void> {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: MAESTRONEXUS_TOOLS.map((tool) => ({
      name: tool.name,
...
Follow these steps:
1. READ \`maestronexus://repo/${repo || '{name}'}/context\` for codebase stats
2. READ \`maestronexus://repo/${repo || '{name}'}/clusters\` to see all functional areas
3. READ \`maestronexus://repo/${repo || '{name}'}/processes\` to see all execution flows  
4. For the top 5 most important processes, READ \`maestronexus://repo/${repo || '{name}'}/process/{name}\` for step-by-step traces
