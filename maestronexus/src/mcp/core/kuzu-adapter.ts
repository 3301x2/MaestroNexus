 ...
 * Retries on lock errors (e.g., when `maestronexus analyze` is running).
 */
export const initKuzu = async (repoId: string, dbPath: string): Promise<void> => {
  ...
  try {
    ...
    throw new Error(`KuzuDB not found at ${dbPath}. Run: maestronexus analyze`);
  }
  ...
  // avoids lock conflicts when `maestronexus analyze` is writing.
