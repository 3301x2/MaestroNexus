 ...
 * 
 * Provides tool implementations using local .maestronexus/ indexes.
...
// AI context generation is CLI-only (maestronexus analyze)
...
export class LocalBackend {
  ...
  async resolveRepo(repoParam?: string): Promise<RepoHandle> {
    ...
    if (this.repos.size === 0) {
      throw new Error('No indexed repositories. Run: maestronexus analyze');
    }
    if (repoParam) {
  ...
  private async bm25Search(repo: RepoHandle, query: string, limit: number): Promise<any[]> {
    ...
    try {
      ...
      console.error('MaestroNexus: BM25/FTS search failed (FTS indexes may not exist) -', err.message);
      return [];
    }
