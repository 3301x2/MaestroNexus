 ...
 * Repository Manager
 * 
 * Manages MaestroNexus index storage in .maestronexus/ at repo root.
 * Also maintains a global registry at ~/.maestronexus/registry.json
...
/**
 * Shape of an entry in the global registry (~/.maestronexus/registry.json)
 */
...
const MAESTRONEXUS_DIR = '.maestronexus';

// ─── Local Storage Helpers ─────────────────────────────────────────────

/**
 * Get the .maestronexus storage path for a repository
 */
export const getStoragePath = (repoPath: string): string => {
  return path.join(path.resolve(repoPath), MAESTRONEXUS_DIR);
};
...
/**
 * Check if a path has a MaestroNexus index
 */
...
/**
 * Find .maestronexus by walking up from a starting path
 */
...
/**
 * Add .maestronexus to .gitignore if not already present
 */
export const addToGitignore = async (repoPath: string): Promise<void> => {
  ...
  try {
    ...
    if (content.includes(MAESTRONEXUS_DIR)) return;
    
    const newContent = content.endsWith('\n') 
      ? `${content}${MAESTRONEXUS_DIR}\n`
      : `${content}\n${MAESTRONEXUS_DIR}\n`;
    await fs.writeFile(gitignorePath, newContent, 'utf-8');
  } catch {
    // .gitignore doesn't exist, create it
    await fs.writeFile(gitignorePath, `${MAESTRONEXUS_DIR}\n`, 'utf-8');
  }
};

// ─── Global Registry (~/.maestronexus/registry.json) ───────────────────────

/**
 * Get the path to the global MaestroNexus directory
 */
export const getGlobalDir = (): string => {
  return path.join(os.homedir(), '.maestronexus');
};
 ...
 * Called after `maestronexus analyze` completes.
 */
 ...
 * Called after `maestronexus clean`.
 */
 ...
 * Optionally validates that each entry's .maestronexus/ still exists.
 */
export const listRegisteredRepos = async (opts?: { validate?: boolean }): Promise<RegistryEntry[]> => {
  const entries = await readRegistry();
  if (!opts?.validate) return entries;

  // Validate each entry still has a .maestronexus/ directory
...
// ─── Global CLI Config (~/.maestronexus/config.json) ─────────────────────────
...
/**
 * Load CLI config from ~/.maestronexus/config.json
 */
...
/**
 * Save CLI config to ~/.maestronexus/config.json
 */
