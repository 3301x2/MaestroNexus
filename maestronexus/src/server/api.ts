...
const buildGraph = async (
  ...
  try {
    ...
    console.warn('MaestroNexus: relationship query failed:', err?.message);
  }
...
export const createServer = async (port: number) => {
  ...
  if (!hasRepos) {
    console.warn('MaestroNexus: No indexed repositories found. The server will start but most endpoints will return errors.');
    console.warn('Run "maestronexus analyze" in a repository to index it first.');
  }
  ...
  app.use(cors({
    origin: (origin, callback) => {
      ...
      if (
        ...
        || origin === 'https://maestronexus.vercel.app'
      ) {
        callback(null, true);
      } else {
  ...
  const server = app.listen(port, '127.0.0.1', () => {
    console.log(`MaestroNexus server running on http://localhost:${port}`);
    console.log(`Serving ${hasRepos ? 'all indexed repositories' : 'no repositories (run maestronexus analyze first)'}`);
  });
