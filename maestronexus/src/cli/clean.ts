 ...
 * Clean Command
 * 
 * Removes the .maestronexus index from the current repository.
...
export const cleanCommand = async (options?: { force?: boolean; all?: boolean }) => {
  ...
  if (options?.all) {
    if (!options?.force) {
      ...
      console.log(`This will delete MaestroNexus indexes for ${entries.length} repo(s):`);
  ...
  if (!options?.force) {
    console.log(`This will delete the MaestroNexus index for: ${repoName}`);
