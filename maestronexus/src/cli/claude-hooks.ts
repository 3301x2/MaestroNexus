 ...
 * 
 * Registers the MaestroNexus PreToolUse hook in ~/.claude/hooks.json
...
/**
 * Get the absolute path to the maestronexus-hook.js file.
 * Works for both local dev and npm-installed packages.
 */
function getHookScriptPath(): string {
  // From dist/cli/claude-hooks.js → hooks/claude/maestronexus-hook.js
  const packageRoot = path.resolve(__dirname, '..', '..');
  return path.join(packageRoot, 'hooks', 'claude', 'maestronexus-hook.cjs');
}

/**
 * Register (or verify) the MaestroNexus hook in Claude Code's global hooks.json.
 * 
 * - Creates ~/.claude/ and hooks.json if they don't exist
 * - Preserves existing hooks from other tools
 * - Skips if MaestroNexus hook is already registered
 * 
...
export async function registerClaudeHook(): Promise<{ registered: boolean; message: string }> {
  ...
  // Check if MaestroNexus hook is already registered
  const existingEntry = hooksConfig.hooks.PreToolUse.find((entry: any) => {
    ...
    return entry.hooks.some((h: any) => 
      h.command && (
        h.command.includes('maestronexus-hook') || 
        h.command.includes('maestronexus augment')
      )
    );
  });
  ...
  // Add the MaestroNexus hook entry
