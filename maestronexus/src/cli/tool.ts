 ...
 * 
 * Exposes MaestroNexus tools (query, context, impact, cypher) as direct CLI commands.
 * Bypasses MCP entirely — invokes LocalBackend directly for minimal overhead.
 * 
 * Usage:
 *   maestronexus query "authentication flow"
 *   maestronexus context --name "validateUser"
 *   maestronexus impact --target "AuthService" --direction upstream
 *   maestronexus cypher "MATCH (n:Function) RETURN n.name LIMIT 10"
 * 
...
async function getBackend(): Promise<LocalBackend> {
  ...
  if (!ok) {
    console.error('MaestroNexus: No indexed repositories found. Run: maestronexus analyze');
    process.exit(1);
  }
  return _backend;
}
...
export async function queryCommand(queryText: string, options?: {
  ...
  if (!queryText?.trim()) {
    console.error('Usage: maestronexus query <search_query>');
    process.exit(1);
  }
...
export async function contextCommand(name: string, options?: {
  ...
  if (!name?.trim() && !options?.uid) {
    console.error('Usage: maestronexus context <symbol_name> [--uid <uid>] [--file <path>]');
    process.exit(1);
  }
...
export async function impactCommand(target: string, options?: {
  ...
  if (!target?.trim()) {
    console.error('Usage: maestronexus impact <symbol_name> [--direction upstream|downstream]');
    process.exit(1);
  }
...
export async function cypherCommand(query: string, options?: {
  ...
  if (!query?.trim()) {
    console.error('Usage: maestronexus cypher <cypher_query>');
    process.exit(1);
  }
