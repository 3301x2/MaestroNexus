 ...
 * 
 * Usage:
 *   maestronexus eval-server                    # default port 4848
 *   maestronexus eval-server --port 4848        # explicit port
 *   maestronexus eval-server --idle-timeout 300 # auto-shutdown after 300s idle
 * 
 * API:
...
function formatContextResult(result: any): string {
  ...
  if (result.status === 'ambiguous') {
    ...
    lines.push(`\nRe-run: maestronexus-context "${result.candidates?.[0]?.name}" "<file_path>"`);
    return lines.join('\n');
  }
...
function getNextStepHint(toolName: string): string {
  switch (toolName) {
    case 'query':
      return '\n---\nNext: Pick a symbol above and run maestronexus-context "<name>" to see all its callers, callees, and execution flows.';

    case 'context':
      return '\n---\nNext: To check what breaks if you change this, run maestronexus-impact "<name>" upstream';

    case 'impact':
      return '\n---\nNext: Review d=1 items first (WILL BREAK). Read the source with cat to understand the code, then make your fix.';

    case 'cypher':
      return '\n---\nNext: To explore a result symbol in depth, run maestronexus-context "<name>"';

    case 'detect_changes':
      return '\n---\nNext: Run maestronexus-context "<symbol>" on high-risk changed symbols to check their callers.';
...
export async function evalServerCommand(options?: EvalServerOptions): Promise<void> {
  ...
  if (!ok) {
    console.error('MaestroNexus eval-server: No indexed repositories found. Run: maestronexus analyze');
    process.exit(1);
  }

  const repos = await backend.listRepos();
  console.error(`MaestroNexus eval-server: ${repos.length} repo(s) loaded: ${repos.map(r => r.name).join(', ')}`);
  ...
  function resetIdleTimer() {
    ...
    idleTimer = setTimeout(async () => {
      console.error('MaestroNexus eval-server: Idle timeout reached, shutting down');
  ...
  server.listen(port, '127.0.0.1', () => {
    console.error(`MaestroNexus eval-server: listening on http://127.0.0.1:${port}`);
    ...
    try {
      process.stdout.write(`MAESTRONEXUS_EVAL_SERVER_READY:${port}\n`);
    } catch {
  ...
  const shutdown = async () => {
    console.error('MaestroNexus eval-server: shutting down...');
    await backend.disconnect();
    server.close();
    process.exit(0);
  };
