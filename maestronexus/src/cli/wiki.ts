 ...
 * Usage: maestronexus wiki [path] [options]
 */
...
export const wikiCommand = async (
  ...
  if (!meta) {
    console.log('  Error: No MaestroNexus index found.');
    console.log('  Run `maestronexus analyze` first to index this repository.\n');
    process.exitCode = 1;
    return;
  }
  ...
  if (options?.apiKey || options?.model || options?.baseUrl) {
    ...
    console.log('  Config saved to ~/.maestronexus/config.json\n');
  }
  ...
  if (!hasSavedConfig && !hasCLIOverrides) {
    if (!process.stdin.isTTY) {
      if (!llmConfig.apiKey) {
        ...
        console.log('  Set OPENAI_API_KEY or MAESTRONEXUS_API_KEY environment variable,');
      ...
      const envKey = process.env.MAESTRONEXUS_API_KEY || process.env.OPENAI_API_KEY || '';
      let key: string;
      if (envKey) {
    ...
    if (err.message?.includes('No source files')) {
      ...
      if (isAuthError && process.stdin.isTTY) {
        ...
        if (!answer || answer === 'y' || answer === 'yes') {
          ...
          await saveCLIConfig({});
          console.log('  Config cleared. Run `maestronexus wiki` again to reconfigure.\n');
        }
      }
    } else {
      console.log(`\n  Error: ${err.message}\n`);
...
function publishGist(htmlPath: string): { url: string; rawUrl: string } | null {
  try {
    const output = execSync(
      `gh gist create "${htmlPath}" --desc "Repository Wiki — generated by MaestroNexus" --public`,
