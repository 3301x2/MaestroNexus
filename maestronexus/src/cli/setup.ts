 ...
 * so the MaestroNexus MCP server is available in all projects.
 */
...
function getMcpEntry() {
  if (process.platform === 'win32') {
    return {
      command: 'cmd',
      args: ['/c', 'npx', '-y', 'maestronexus@latest', 'mcp'],
    };
  }
  return {
    command: 'npx',
...
/**
 * Merge maestronexus entry into an existing MCP config JSON object.
...
function mergeMcpConfig(existing: any): any {
  ...
  existing.mcpServers.maestronexus = getMcpEntry();
  return existing;
}
...
async function setupClaudeCode(result: SetupResult): Promise<void> {
  ...
  console.log('');
  console.log('  Claude Code detected. Run this command to add MaestroNexus MCP:');
  console.log('');
  console.log('    claude mcp add maestronexus -- npx -y maestronexus mcp');
  console.log('');
  result.configured.push('Claude Code (MCP manual step printed)');
}

/**
 * Install MaestroNexus skills to ~/.claude/skills/ for Claude Code.
 */
...
/**
 * Install MaestroNexus hooks to ~/.claude/settings.json for Claude Code.
...
async function installClaudeCodeHooks(result: SetupResult): Promise<void> {
  ...
  // Source hooks bundled within the maestronexus package (hooks/claude/)
  const pluginHooksPath = path.join(__dirname, '..', '..', 'hooks', 'claude');

  // Copy unified hook script to ~/.claude/hooks/maestronexus/
  const destHooksDir = path.join(claudeDir, 'hooks', 'maestronexus');

  try {
    await fs.mkdir(destHooksDir, { recursive: true });

    const src = path.join(pluginHooksPath, 'maestronexus-hook.cjs');
    const dest = path.join(destHooksDir, 'maestronexus-hook.cjs');
    try {
    ...
    const hookCmd = `node "${path.join(destHooksDir, 'maestronexus-hook.cjs').replace(/\\/g, '/')}"`;
    ...
    const hasPreToolHook = existing.hooks.PreToolUse.some(
      (h: any) => h.hooks?.some((hh: any) => hh.command?.includes('maestronexus'))
    );
    if (!hasPreToolHook) {
      existing.hooks.PreToolUse.push({
        ...
        hooks: [{
          ...
          command: hookCmd,
          timeout: 8000,
          statusMessage: 'Enriching with MaestroNexus graph context...',
        }],
      });
    }
...
async function setupOpenCode(result: SetupResult): Promise<void> {
  ...
  try {
    ...
    config.mcp.maestronexus = getMcpEntry();
...
const SKILL_NAMES = ['maestronexus-exploring', 'maestronexus-debugging', 'maestronexus-impact-analysis', 'maestronexus-refactoring', 'maestronexus-guide', 'maestronexus-cli'];

/**
 * Install MaestroNexus skills to a target directory.
 * Each skill is installed as {targetDir}/maestronexus-{skillName}/SKILL.md
...
/**
 * Install global Cursor skills to ~/.cursor/skills/maestronexus/
 */
...
/**
 * Install global OpenCode skills to ~/.config/opencode/skill/maestronexus/
 */
...
export const setupCommand = async () => {
  console.log('');
  console.log('  MaestroNexus Setup');
  console.log('  ==============');
  console.log('');
  ...
  console.log('    2. Run: maestronexus analyze');
